...
namespace Paup_2021_ServisVozila.Reports
{
    public class VozilaReport
    {
        ...
        public void IspisVozila(List<Automobili> a, LogiraniKorisnik k)
        {
            ...
            using (MemoryStream mstream = new MemoryStream())
            {
                using (Document pdfDokument = new Document(PageSize.A4, 50, 50, 20, 50))
                {
                    ...
                    Paragraph pNaslov = new Paragraph("Popis vozila", fontNaslov);
                    ...
                    x.SetWidths(new float[] { 5, 3, 3, 3, 3 });
                    x.WidthPercentage = 100;
                    x.SpacingBefore = 10;

                    x.AddCell(GenerirajCeliju("VIN: ", fontTekstBold, tPozadinaSadrzaj, true));
                    x.AddCell(GenerirajCeliju("Vlasnik: ", fontTekstBold, tPozadinaSadrzaj, true));
                    x.AddCell(GenerirajCeliju("Proizvodac: ", fontTekstBold, tPozadinaSadrzaj, true));
                    x.AddCell(GenerirajCeliju("Model: ", fontTekstBold, tPozadinaSadrzaj, true));
                    x.AddCell(GenerirajCeliju("Registracija: ", fontTekstBold, tPozadinaSadrzaj, true));

                    foreach (var auto in a)
                    {
                        x.AddCell(GenerirajCeliju(auto.Vin, fontTekst, tPozadinaSadrzaj, false));
                        x.AddCell(GenerirajCeliju(auto.Vlasnik.PrezimeIme, fontTekst, tPozadinaSadrzaj, false));
                        x.AddCell(GenerirajCeliju(auto.marka.Marke, fontTekst, tPozadinaSadrzaj, false));
                        x.AddCell(GenerirajCeliju(auto.Model, fontTekst, tPozadinaSadrzaj, false));
                        x.AddCell(GenerirajCeliju(auto.Registracija, fontTekst, tPozadinaSadrzaj, false));
                    }
